...
import { FlakeLightController } from './FlakeLightController';
...
export class Stage {
  ...
  // Flake lights system (now managed by FlakeLightController)
  private flakeLightController!: FlakeLightController;
   ...
   * Now managed by FlakeLightController
   */
  private createFlakeLights(): void {
    // Create controller with stage configuration
    this.flakeLightController = new FlakeLightController(this.scene, {
      count: this.FLAKE_COUNT,
      stageWidth: this.config.width * 1.5,
      stageDepth: this.config.depth * 2,
      maxHeight: this.config.trussHeight * 1.3,
    });
  }
  ...
  updateFlakeLights(frame: AudioFrame): void {
    if (!this.flakeLightController) return;
    this.flakeLightController.update(frame);
  }

  /**
   * Gets the flake light controller
   */
  getFlakeLightController(): FlakeLightController {
    return this.flakeLightController;
  }
  ...
  update(deltaTime: number): void {
    ...
    // Flake lights animation is now handled inside FlakeLightController.update()
    // No separate animation update needed
  }
  ...
  /**
   * Gets the LED particle panel
   * @returns LED particle panel or undefined
   */
  ...
  dispose(): void {
    ...
    // Dispose flake light controller
    if (this.flakeLightController) {
      this.flakeLightController.dispose();
    }

    // Dispose LED particle panel
    if (this.ledParticlePanel) {
      this.ledParticlePanel.dispose();
    }
